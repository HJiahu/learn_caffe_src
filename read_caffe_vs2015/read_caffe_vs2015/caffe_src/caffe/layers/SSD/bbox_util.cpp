...
namespace caffe
{
                             ...
                             const EmitConstraint& emit_constraint)
    {
        ...
        if (emit_type == EmitConstraint_EmitType_CENTER)
        {
            ...
            if (emit_type == EmitConstraint_EmitType_MIN_OVERLAP)
            {
        ...
        const NormalizedBBox& bbox, NormalizedBBox* encode_bbox)
    {
        if (code_type == PriorBoxParameter_CodeType_CORNER)
        {
            ...
            if (code_type == PriorBoxParameter_CodeType_CENTER_SIZE)
            {
                ...
                if (code_type == PriorBoxParameter_CodeType_CORNER_SIZE)
                {
                    float prior_width = prior_bbox.xmax() - prior_bbox.xmin();
                    CHECK_GT (prior_width, 0);
                    float prior_height = prior_bbox.ymax() - prior_bbox.ymin();
                ...
                for (int j = 0; j < match_index.size(); ++j)
                {
                    ...
                    for (int k = 0; k < 4; ++k)
                    {
                        ...
                        if (loc_loss_type == MultiBoxLossParameter_LocLossType_SMOOTH_L1)
                        {
                            ...
                            if (loc_loss_type == MultiBoxLossParameter_LocLossType_L2)
                            {
                                loss += 0.5 * val * val;
                            }
        ...
        for (int i = 0; i < num; ++i)
        {
            ...
            for (int p = 0; p < num_preds_per_class; ++p)
            {
                ...
                if (loss_type == MultiBoxLossParameter_ConfLossType_SOFTMAX)
                {
                    ...
                    if (loss_type == MultiBoxLossParameter_ConfLossType_LOGISTIC)
                    {
                        int target = 0;
                    ...
                    it != score_index_vec.end();)
            {
                ...
                if (reuse_overlaps)
                {
                            ...
                            (*overlaps) [best_idx].end())
                    {
                        ...
                        if (overlaps->find (cur_idx) != overlaps->end() &&
